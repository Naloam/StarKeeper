  ...
  const tabs = [
    {
      id: "abandoned",
      label: "废弃项目",
      count: analysis.categories.abandoned.length,
      icon: TrendingDown,
      color: "red",
    },
    {
      id: "similar",
      label: "相似项目",
      count: analysis.categories.similar.length,
      icon: Copy,
      color: "yellow",
    },
    {
      id: "lowEngagement",
      label: "低交互",
      count: analysis.categories.lowEngagement.length,
      icon: Clock,
      color: "gray",
    },
    {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900">发现 {repos.length} 个废弃项目</h2>
        <button
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 text-sm text-gray-500 mb-3">
                  ...
                  {repo.language && (
                    <span className="px-2 py-0.5 bg-gray-100 rounded">{repo.language}</span>
                  )}
                ...
                >
                  <AlertTriangle
                    className={`w-4 h-4 mt-0.5 ${repo.severity === "high" ? "text-red-600" : "text-yellow-600"}`}
                  />
                  <div className="flex-1">
                    <div
                      className={`text-sm font-medium ${repo.severity === "high" ? "text-red-900" : "text-yellow-900"}`}
                    >
                      {repo.reason}
                    </div>
                    <div
                      className={`text-sm mt-1 ${repo.severity === "high" ? "text-red-700" : "text-yellow-700"}`}
                    >
                      {repo.recommendation}
                    </div>
                  </div>
...
function SimilarReposView({ groups, metadata, selectedRepos, onSelect, onSelectAll }) {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-semibold text-gray-900">发现 {groups.length} 组相似项目</h2>
...
function LowEngagementReposView({ repos, metadata, selectedRepos, onSelect, onSelectAll }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900">发现 {repos.length} 个低交互项目</h2>
        <button
          onClick={() => onSelectAll(repos)}
          className="text-sm text-primary-600 hover:text-primary-700 font-medium"
        >
...
function ArchivedReposView({ repos, metadata, selectedRepos, onSelect, onSelectAll }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900">{repos.length} 个已归档项目</h2>
        <button
          onClick={() => onSelectAll(repos)}
          className="text-sm text-primary-600 hover:text-primary-700 font-medium"
        >
